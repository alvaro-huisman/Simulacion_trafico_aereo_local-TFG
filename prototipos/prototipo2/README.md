...
- Resultados combinados: `prototipos/prototipo2/salidas/resultados/resultados_p2.csv`
...
- **Generacion de vuelos**:
  - `total_vuelos_diarios`: numero total de vuelos que se distribuiran por pesos.
  ...
  - `prob_destino_exterior`: % de vuelos que van al nodo externo `EXTERIOR` (consumen capacidad en origen).
...
- **Simulacion**:
  ...
  - `tmin_fase_asc_des_min` / `tmin_fase_crucero_min`: duraciones minimas por fase para evitar vuelos irreales.
  ...
  - Nota: la configuracion por defecto busca un equilibrio razonable (capacidad moderada, ~600 vuelos diarios, umbral de espera 30 min y ruido exterior moderado). Sube/baja estas cifras segun quieras mas/menos congestion y redirecciones.
